 ...
 */
import { requireUser, requireRegistered, publicUser, bearerToken, findBusinessMeta, markPriority, realmIdOf, isSuperAdmin } from '../guards.js';
...
async function handleMe({ request, env }) {
  ...
  const meta = await findBusinessMeta(env, user.business, realmIdOf(user));
  ...
  return publicUser(user, { court: meta.court, hold: meta.hold, superAdmin: isSuperAdmin(env, user) });
}
