...
export async function logAudit(env, { actor, business, action, detail, realmId }) {
  try {
    const db = await getDb(env);
    await db.prepare('INSERT INTO audit (realm_id, ts, actor, actor_business, action, detail) VALUES (?, ?, ?, ?, ?, ?)')
      .bind(String(realmId || 'default'), new Date().toISOString(), String(actor || ''), String(business || ''), String(action || ''), String(detail || '')).run();
 ...
 */
export async function listAudit(env, { limit = 150, actor, action, from, to, realmId } = {}) {
  const db = await getDb(env);
  const where = ['realm_id = ?'];
  const binds = [String(realmId || 'default')];
...
export async function listAuditActions(env, realmId) {
  const db = await getDb(env);
  const { results } = await db.prepare('SELECT DISTINCT action FROM audit WHERE realm_id = ? ORDER BY action')
    .bind(String(realmId || 'default')).all();
