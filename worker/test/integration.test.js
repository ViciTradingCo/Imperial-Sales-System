...
describe('transfers conserve stock', () => {
  it('accept moves stock from sender to receiver', async () => {
    ...
    await createTransfer(env, 'Alpha', { toBusiness: 'Beta', item: 'Iron Sword', qty: 3 });
    expect(await stockOf('Alpha', 'Iron Sword')).toBe(7); // debited on send

    const { incoming } = await listTransfers(env, 'Beta');
    expect(incoming).toHaveLength(1);
    await acceptTransfer(env, 'Beta', incoming[0].id);
  ...
  it('decline returns the goods to the sender', async () => {
    await seedItem('Alpha', 'Iron Sword', 30, 10);
    await createTransfer(env, 'Alpha', { toBusiness: 'Beta', item: 'Iron Sword', qty: 4 });
    expect(await stockOf('Alpha', 'Iron Sword')).toBe(6);
    const { incoming } = await listTransfers(env, 'Beta');
    await declineTransfer(env, 'Beta', incoming[0].id);
  ...
  it('cancel by the sender returns the goods', async () => {
    await seedItem('Alpha', 'Iron Sword', 30, 10);
    await createTransfer(env, 'Alpha', { toBusiness: 'Beta', item: 'Iron Sword', qty: 5 });
    const { outgoing } = await listTransfers(env, 'Alpha');
    await cancelTransfer(env, 'Alpha', outgoing[0].id);
  ...
  it('rejects a transfer bigger than stock', async () => {
    ...
    await expect(createTransfer(env, 'Alpha', { toBusiness: 'Beta', item: 'Iron Sword', qty: 5 }))
  ...
  it('is idempotent on a repeated send key', async () => {
    await seedItem('Alpha', 'Iron Sword', 30, 10);
    await createTransfer(env, 'Alpha', { toBusiness: 'Beta', item: 'Iron Sword', qty: 3, idempotencyKey: 'k1' });
    await createTransfer(env, 'Alpha', { toBusiness: 'Beta', item: 'Iron Sword', qty: 3, idempotencyKey: 'k1' });
...
describe('coffers track balance', () => {
  it('deposits and withdrawals sum to the balance', async () => {
    await adjustCoffer(env, 'Alpha', { amount: 100, note: 'seed' });
    await adjustCoffer(env, 'Alpha', { amount: -30, note: 'buy' });
    expect(await cofferBalance(env, 'Alpha')).toBe(70);
  });

  it('rejects a zero adjustment', async () => {
    await expect(adjustCoffer(env, 'Alpha', { amount: 0 })).rejects.toThrow(/non-zero/i);
  });
});
