/**
 * Loads the PUBLIC app config (OAuth client ID, API URL) once. Nothing secret
 * lives here — see app-config.json. Fetched at runtime rather than bundled so
 * the same build can be pointed at a different API by editing one JSON file.
 */
let cached = null;
...
export async function loadConfig() {
  ...
  const missing = ['googleClientId', 'apiBaseUrl']
